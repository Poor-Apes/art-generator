...
def getFontWithPath(font_name):
    return os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../Fonts/" + font_name + ".ttf")
    )


def getImageWithPath(image_full_name):
    return os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../Cards/" + image_full_name)
    )


def getFinalCardsFrontPath(card_number):
    return os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "../../IPFS/Cards/" + str(card_number) + ".png"
        )
    )
...
def create_card(
    ...
    # NFT Number
    drawable_image.text(
        (160, 240), "105", fill=gravitas_one_colour, font=gravitas_one_100
    )
    ...
    # Date of Birth
    drawable_image.text(
        (1500, 435), "12/10/1932", fill=rock_salt_colour, font=rock_salt_70
    )
    ...
    # Local
    drawable_image.text(
        (120, 717), "LOCATION: Shops", fill=typewriter_colour, font=typewriter_80
    )

    # Clothes
    drawable_image.text(
        (135, 817), "CLOTHES: Dungarees", fill=typewriter_colour, font=typewriter_80
    )

    # Head
    drawable_image.text(
        (120, 910), "HEAD: Flat Cap", fill=typewriter_colour, font=typewriter_80
    )

    # Eyes
    drawable_image.text(
        (1200, 710), "EYES: Yellow Glasses", fill=typewriter_colour, font=typewriter_80
    )

    # Mouth
    drawable_image.text(
        (1180, 822), "MOUTH: Cigarette", fill=typewriter_colour, font=typewriter_80
    )

    registration_card.save(getFinalCardsFrontPath())
