...
parser.add_argument(
    "-s", "--season", required=True, help="The season (chicago, new_york, detroit)"
)
parser.add_argument(
    "-n",
    "--nft-folder-hash",
    required=True,
    dest="nft_folder_hash",
    help="The hash of the NFT folder on IPFS",
)
parser.add_argument(
    "-c",
    "--card-folder-hash",
    required=True,
    dest="card_folder_hash",
    help="The hash of the card folder on IPFS",
)
parser.add_argument(
    "-cf",
    "--card-file-hash",
    required=True,
    dest="card_file_hash",
    help="The hash of the card viewer HTML file on IPFS",
)
...
chicago_background_colours = ["966a64", "7e6254", "616b63", "89815a"]
...
with open(
    os.path.join(json_folder_path, args.season + "_manifest") + ".json", "r"
) as file:
    ...
    for current_json in progressbar(manifest, "Generating JSON files: ", 40):
        ...
        data["image"] = (
            ipfs_url + args.nft_folder_hash + "/" + str(current_file_number) + ".png"
        )
        data["external_url"] = (
            ipfs_url + args.card_file_hash + "/?number=" + str(current_file_number)
        )
        data["background_color"] = chicago_background_colours[
            current_json["background"]
        ]
        data["attributes"] = {
            "card": ipfs_url
            + args.card_folder_hash
            + "/"
        ...
        with open(
            os.path.join(
                json_folder_path, args.season.title(), str(current_file_number)
            ),
            "w",
        ) as current_json_file:
